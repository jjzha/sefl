...
"""
Example usage:
   python command-r_as_a_judge.py --input_csv annotation_file.csv \
                        --output_csv results_gpt4.csv
   (Ensure COHERE_API_KEY is set in the environment.)
"""
...
def parse_args():
    ...
    parser.add_argument("--gpt_model", type=str, default="command-r-plus-08-2024",
                        help="model name.")
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            raw_text = response.message.content[-1].text.strip()

        except Exception as e:
            print(f"Error calling COHERE API on row {idx}: {e}")
            raw_text = "ERROR"
